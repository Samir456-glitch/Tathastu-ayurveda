...
export default function Home() {
  ...
  const [doctorList, setDoctorList] = useState(["Dr. Sumer Kumar Sharma"]);
  const [medicineCategories, setMedicineCategories] = useState(DEFAULT_CATEGORIES);

  // Master Management State
  const [masterType, setMasterType] = useState("medicine");
  ...
  async function loadMasterPresets() {
    try {
      ...
      if (data && data.length > 0) {
        ...
        const docs = ["Dr. Sumer Kumar Sharma"];
  ...
  // =========================
  // 1. HOME SCREEN (Clean Front Page)
  // =========================
  if (screen === "home") {
    return (
      <main style={{ minHeight: "100vh", background: "#f5f7f2", padding: "20px", fontFamily: "Arial, sans-serif" }}>
        ...
        {/* Clean 3-Column Clinical Counters (No Fee Shown) */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px", maxWidth: "460px", margin: "14px 0" }}>
          ...
          <div onClick={() => openPatientList("Waiting")} style={{ background: "#fff8e1", border: "1.5px solid #ffe082", padding: "10px 6px", borderRadius: "8px", textAlign: "center", cursor: "pointer" }}>
            <div style={{ fontSize: "12px", color: "#f57f17", fontWeight: "bold" }}>⏳ प्रतीक्षारत (Waiting)</div>
  ...
  // =========================
  // 2. MASTER SETTINGS SCREEN (No-Code Master Manager)
  // =========================
  if (screen === "settingsScreen") {
    return (
      <main style={{ minHeight: "100vh", background: "#f5f7f2", padding: "20px", fontFamily: "Arial, sans-serif" }}>
        ...
        {/* 1. Master Clinical Presets (No-Code Adder) */}
        <div style={{ background: "#fff", padding: "16px", borderRadius: "10px", border: "1.5px solid #81c784", maxWidth: "520px", marginBottom: "20px" }}>
          <h3 style={{ margin: "0 0 8px 0", color: "#2e7d32" }}>🌿 नया विकल्प जोड़ें (Quick Preset Manager)</h3>
          <p style={{ fontSize: "12px", color: "#666", margin: "0 0 12px 0" }}>यहाँ से आप बिना कोड छुए नई औषधि, अनुपान, मात्रा, डॉक्टर या टेस्ट सूची में जोड़ सकते हैं:</p>

          <div style={{ display: "grid", gap: "8px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
              <select value={masterType} onChange={(e) => setMasterType(e.target.value)} style={{ padding: "8px", borderRadius: "4px", border: "1px solid #ccc" }}>
              ...
              {masterType === "medicine" ? (
                <select value={newMasterCategory} onChange={(e) => setNewMasterCategory(e.target.value)} style={{ padding: "8px", borderRadius: "4px", border: "1px solid #ccc" }}>
                ...
                <div style={{ fontSize: "12px", color: "#666", alignSelf: "center" }}>सामान्य सूची</div>
              )}
            </div>

            <input
              placeholder="नाम लिखें (उदा. गिलोय चूर्ण / गोखरू क्वाथ)..."
              value={newMasterVal}
            ...
            <button onClick={addMasterPreset} style={{ padding: "10px", background: "#2e7d32", color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer" }}>
              ➕ सूची में सुरक्षित जोड़ें
            </button>
          </div>
        </div>

        {/* 2. Hospital Details & Letterhead */}
        ...
        <div style={{ background: "#fff", padding: "18px", borderRadius: "12px", maxWidth: "600px", border: "1px solid #ddd" }}>
          ...
          <div style={{ marginBottom: "12px" }}>
            ...
            <select
              ...
              style={{ width: "100%", padding: "8px", borderRadius: "4px", border: "1px solid #ccc", boxSizing: "border-box", background: "#fff" }}
            >
          ...
          {/* Categorized Tabs (Populated Dynamically) */}
          <div style={{ marginBottom: "12px" }}>
          ...
          {/* Investigation Section (Populated Dynamically) */}
